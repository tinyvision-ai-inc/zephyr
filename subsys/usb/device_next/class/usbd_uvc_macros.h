...
#define NODE_ID(entity) DT_DEP_ORD(entity)
#define LOOKUP_ID(node, compat) 0
...
#define VC_SOURCE_IDS(entity)							\
	DT_FOREACH_CHILD(DT_CHILD(entity, port), VC_SOURCE_ID)
...
#define VC_COND_VIDEO_CONTROLS(node, fn)                                        \
	IF_ENABLED(DT_NODE_HAS_PROP(node, video_controls), (fn(node)))
#define VC_FOREACH_ENTITY(fn)                                                   \
	DT_FOREACH_STATUS_OKAY_NODE_VARGS(VC_COND_VIDEO_CONTROLS, fn)

#define VS_INST_COMPAT(n, compat, fn) fn(DT_INST(inst, compat))
#define VS_FOREACH_FORMAT(fn)                                                   \
	DT_COMPAT_FOREACH_STATUS_OKAY_VARGS(zephyr_uvc_format_uncompressed,	\
					    VS_INST_COMPAT, fn)			\
	DT_COMPAT_FOREACH_STATUS_OKAY_VARGS(zephyr_uvc_format_mjpeg,		\
					    VS_INST_COMPAT, fn)
...
#define VC_DESCRIPTOR(entity)							\
	IF_ENABLED(VC_IS_IT(entity), (VC_CT_DESCRIPTOR(entity)))		\
	IF_ENABLED(VC_IS_OT(entity), (VC_OT_DESCRIPTOR(entity)))		\
	IF_ENABLED(VC_IS_PU(entity), (VC_PU_DESCRIPTOR(entity)))		\
	IF_ENABLED(VC_IS_SU(entity), (VC_SU_DESCRIPTOR(entity)))
//	IF_ENABLED(VC_IS_EU(entity), (VC_EU_DESCRIPTOR(entity)))
//	IF_ENABLED(VC_IS_XU(entity), (VC_XU_DESCRIPTOR(entity)))

/* Automatically assign format IDs based on format order in devicetree */
#define VS_FORMAT_ID(fmt)							\
	(NODE_ID(fmt) - LOOKUP_ID(DT_PARENT(fmt), zephyr_uvc_control_ot))

#define VS_NODE_IS_FORMAT(node)							\
	UTIL_OR(DT_NODE_HAS_COMPAT(node, zephyr_uvc_format_mjpeg),		\
		DT_NODE_HAS_COMPAT(node, zephyr_uvc_format_uncompressed))

/* Compute the bNumFormats field value */
#define ZERO(...) 0x00,
#define VS_NUM_FORMATS(node) sizeof((uint8_t []){VS_FOREACH_FORMAT(ZERO)})

/* Compute the total length of all VideoStreaming descriptors */
...
#define VS_TOTAL_LENGTH(node) sizeof((uint8_t []){VS_FOREACH_FORMAT(VS_DESCRIPTOR)})
...
#define VC_OT_DESCRIPTOR(entity)						\
	...
	0x00,						/* bAssocTerminal */	\
	VC_SOURCE_IDS(entity)				/* bSourceID */		\
	0x00,						/* iTerminal */
...
#define VC_SU_DESCRIPTOR(entity)						\
	...
	VC_SOURCE_IDS(entity)				/* baSourceID */	\
	0x00,						/* iSelector */
...
#define VS_UNCOMPRESSED_FORMAT_DESCRIPTOR(fmt)					\
	27,						/* bLength */		\
	USB_DESC_CS_INTERFACE,				/* bDescriptorType */	\
	VS_FORMAT_UNCOMPRESSED,				/* bDescriptorSubtype */\
	VS_FORMAT_ID(fmt),				/* bFormatIndex */	\
	DT_CHILD_NUM(fmt),				/* bNumFrameDescriptors */\
	GUID(fmt),					/* guidFormat */	\
	DT_PROP(fmt, bits_per_pixel),			/* bBitsPerPixel */	\
...
#define VS_MJPEG_FORMAT_DESCRIPTOR(fmt)						\
	11,						/* bLength */		\
	USB_DESC_CS_INTERFACE,				/* bDescriptorType */	\
