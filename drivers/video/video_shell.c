...
static char video_shell_ctrl_name[CONFIG_VIDEO_CTRL_NAME_MAX_SIZE];

/* A global variable holding the buffer set to entry->syntax for control menu values */
static char video_shell_menu_name[CONFIG_VIDEO_CTRL_NAME_MAX_SIZE];
...
static int video_shell_get_ctrl_by_name(const struct device *dev, struct video_ctrl_query *cq,
					char const *name)
{
	char ctrl_name[CONFIG_VIDEO_CTRL_NAME_MAX_SIZE];
	int ret;
	...
	for (size_t i = 0;; i++) {
		...
		video_convert_ctrl_name(name, ctrl_name, sizeof(ctrl_name));
				...
				char **argv)
{
	...
	char menu_name[CONFIG_VIDEO_CTRL_NAME_MAX_SIZE];
	char *arg1 = argv[1];
	size_t i;
	int ret;
	...
	case VIDEO_CTRL_TYPE_MENU:
		for (i = 0; cq.menu[i] != NULL; i++) {
			video_convert_ctrl_name(cq.menu[i], menu_name, sizeof(menu_name));
...
static int video_shell_print_ctrls(const struct shell *sh, const struct device *dev)
{
	...
	while (video_query_ctrl(dev, &cq) == 0) {
		...
		video_convert_ctrl_name(cq.name, ctrl_name, sizeof(ctrl_name));
		cq.name = ctrl_name;
...
static void complete_video_ctrl_menu_name(size_t idx, struct shell_static_entry *entry, int ctrln)
{
	...
	for (size_t i = 0; cq.menu[i] != NULL; i++) {
		if (i == idx) {
			video_convert_ctrl_name(cq.menu[i], video_shell_menu_name,
...
static void complete_video_ctrl_name_dev(size_t idx, struct shell_static_entry *entry, int devn)
{
	...
	video_convert_ctrl_name(video_shell_cq.name, video_shell_ctrl_name,
