...
static int si5351_init(const struct device *dev)
{
	...
	for (int i = 0; i < ARRAY_SIZE(si5351_register_map); i++) {
		uint8_t addr = si5351_register_map[i].addr;
		uint8_t data = si5351_register_map[i].data;

		ret = i2c_reg_write_byte_dt(&config->i2c, addr, data);
		if (ret != 0)
			break;
	}
	return ret;
...
static enum clock_control_status si5351_get_status(const struct device *dev, clock_control_subsys_t sys)
{
...
#define SI5351_DEVICE_DEFINE(n)							\
	...
	const struct si5351_config si5351_config_##n = {			\
		.i2c = I2C_DT_SPEC_INST_GET(n),					\
										...
										\
	DEVICE_DT_INST_DEFINE(n, si5351_init, NULL, NULL, &si5351_config_##n,	\
		PRE_KERNEL_1, CONFIG_CLOCK_CONTROL_INIT_PRIORITY, &si5351_api);
