...
struct uvc_conf {
	...
	struct uvc_desc_entity *const *ctl_desc;
	struct usbd_desc_node *const *str_desc;
...
static int uvc_control_probe(const struct device *dev, uint8_t request, struct uvc_probe *probe)
{
	static int (*control_fn[])(const struct device *, uint8_t, struct uvc_probe *) = {
		uvc_control_probe_format_index,     uvc_control_probe_frame_index,
		uvc_control_probe_frame_interval,   uvc_control_probe_max_video_frame_size,
...
static int uvc_init(struct usbd_class_data *const c_data)
{
	...
	ret = uvc_control_probe(dev, GET_CUR, &data->default_probe);
	if (ret < 0) {
		LOG_ERR("init: failed to query the default probe");
		return ret;
	}

	/* Fill the string desciptors fields. */
	for (int i = 0; conf->str_desc[i] != NULL; i++) {
		struct usbd_desc_node *desc_nd = conf->str_desc[i];
		struct uvc_desc_entity *desc = conf->ctl_desc[i];
...
static int uvc_queue_vbuf(const struct device *dev, struct video_buffer *vbuf)
{
	...
	LOG_DBG("Queue vbuf %p, offset %u/%u, flags 0x%02x", vbuf, data->xfer_offset, xfer_len,
		vbuf->flags);
...
#define UVC_CAP(frame, format)                                                                     \
	{.pixelformat = video_fourcc(DT_PROP(format, fourcc)[0], DT_PROP(format, fourcc)[1],       \
	 ...
	 .height_step = 0},
#define UVC_CAPS(format) VS_FOREACH_CHILD_VARGS(format, UVC_CAP, format)
static const struct video_format_cap uvc_caps[] = {
	//VS_FOREACH_FORMAT(DT_DRV_INST(0), UVC_CAPS) {0},
};
...
#define UVC_CONTROL_MASK(node) DT_STRING_UPPER_TOKEN_BY_IDX(node, compatible, 0)(node, CONTROL)
static const struct uvc_control uvc_controls[] = {
	//VC_FOREACH_ENTITY(DT_DRV_INST(0), UVC_CONTROL) {0},
};
...
#define UVC_FORMATS(format) DT_FOREACH_CHILD_VARGS(format, UVC_FORMAT, format)
static const struct uvc_format uvc_formats[] = {
	//VS_FOREACH_FORMAT(UVC_FORMATS) {0},
};

#define VS_DESCRIPTOR_ARRAY(fmt)                                                                   \
	IF_COMPAT(fmt, zephyr_uvc_format_mjpeg,                                                    \
		  (VS_MJPEG_FORMAT_DESCRIPTOR_ARRAY(fmt)                                           \
			   DT_FOREACH_CHILD(fmt, VS_MJPEG_FRAME_DESCRIPTOR_ARRAY)))                \
	IF_COMPAT(fmt, zephyr_uvc_format_uncompressed,                                             \
		  (VS_UNCOMPRESSED_FORMAT_DESCRIPTOR_ARRAY(fmt)                                    \
			   DT_FOREACH_CHILD(fmt, VS_UNCOMPRESSED_FRAME_DESCRIPTOR_ARRAY)))

#define VS_MJPEG_FRAME_DESCRIPTOR_ARRAY(frame)                                                     \
	static const uint8_t DT_CAT(frame_, desc)[] = {                                            \
		VS_MJPEG_FRAME_DESCRIPTOR(frame)                                                   \
	};

#define VS_UNCOMPRESSED_FRAME_DESCRIPTOR_ARRAY(frame)                                              \
	static const uint8_t DT_CAT(frame, _desc)[] = {                                            \
		VS_UNCOMPRESSED_FRAME_DESCRIPTOR(frame)                                            \
	};

#define VS_UNCOMPRESSED_FORMAT_DESCRIPTOR_ARRAY(fmt)                                               \
	static const uint8_t DT_CAT(fmt, _desc)[] = {                                              \
		VS_UNCOMPRESSED_FORMAT_DESCRIPTOR(fmt)                                             \
	};

#define VS_MJPEG_FORMAT_DESCRIPTOR_ARRAY(fmt)                                                      \
	static const uint8_t DT_CAT(fmt, _desc)[] = {                                              \
		VS_MJPEG_FORMAT_DESCRIPTOR(fmt)                                                    \
	};

#define VC_DESCRIPTOR_ARRAY(entity)                                                                \
	static uint8_t DT_CAT(entity, _desc)[] = {                                                 \
		VC_DESCRIPTOR(entity)                                                              \
	};

#define VC_DESCRIPTOR_STRING(node)                                                                 \
	USBD_DESC_STRING_DEFINE(node##_str, DT_NODE_FULL_NAME(node), USBD_DUT_STRING_INTERFACE);

static uint8_t uvc_desc_iad[] = {
	INTERFACE_ASSOCIATION_DESCRIPTOR(DT_DRV_INST(0))
};

static uint8_t uvc_desc_if_vc[] = {
	VC_INTERFACE_DESCRIPTOR(DT_DRV_INST(0))
};

static uint8_t uvc_desc_if_vc_header[] = {
	VC_INTERFACE_HEADER_DESCRIPTOR(DT_DRV_INST(0))
};

static uint8_t uvc_desc_if_vs[] = {
	VS_INTERFACE_DESCRIPTOR(DT_DRV_INST(0))
};

static uint8_t uvc_desc_if_vs_header[] = {
	VS_INPUT_HEADER_DESCRIPTOR(DT_DRV_INST(0))
};

static uint8_t uvc_fs_desc_ep[] = {
	VS_FULLSPEED_BULK_ENDPOINT_DESCRIPTOR(DT_DRV_INST(0))
};

static uint8_t uvc_hs_desc_ep[] = {
	VS_HIGHSPEED_BULK_ENDPOINT_DESCRIPTOR(DT_DRV_INST(0))
};

static uint8_t uvc_color_format_desc[] = {
	VS_COLOR_MATCHING_DESCRIPTOR(unused)
};

VC_FOREACH_ENTITY(VC_DESCRIPTOR_ARRAY)
VS_FOREACH_FORMAT(VS_DESCRIPTOR_ARRAY)
VC_FOREACH_ENTITY(VC_DESCRIPTOR_STRING)

#define DESCRIPTOR_PTR(node) (void *)DT_CAT(node, _desc),
#define STRING_PTR(node) (struct usbd_desc_node *)&DT_CAT(node, _str),

#define VS_DESCRIPTOR_PTRS(fmt)                                                                    \
	IF_ENABLED(VS_NODE_IS_FORMAT(fmt), (                                                       \
		(struct usb_desc_header *)DT_CAT(fmt, _desc),			                   \
		DT_FOREACH_CHILD(fmt, DESCRIPTOR_PTR)				                   \
		(struct usb_desc_header *)uvc_color_format_desc,		                   \
	))

static struct usb_desc_header *uvc_fs_desc[] = {
	(struct usb_desc_header *)uvc_desc_iad,
	(struct usb_desc_header *)uvc_desc_if_vc,
	(struct usb_desc_header *)uvc_desc_if_vc_header,
	VC_FOREACH_ENTITY(DESCRIPTOR_PTR)
	(struct usb_desc_header *) uvc_desc_if_vs,
	(struct usb_desc_header *)uvc_desc_if_vs_header,
	VS_FOREACH_FORMAT(VS_DESCRIPTOR_PTRS)
	(struct usb_desc_header *) uvc_fs_desc_ep,
	NULL,
};
...
static struct usb_desc_header *uvc_hs_desc[] = {
	...
	(struct usb_desc_header *)uvc_desc_if_vs_header,
	VS_FOREACH_FORMAT(VS_DESCRIPTOR_PTRS)
	(struct usb_desc_header *) uvc_hs_desc_ep,
	NULL,
};

static struct usbd_desc_node *const uvc_str_desc[] = {
	VC_FOREACH_ENTITY(STRING_PTR)
	NULL,
};

static struct uvc_desc_entity *uvc_ctl_desc[] = {
	VC_FOREACH_ENTITY(DESCRIPTOR_PTR)
	NULL,
};

static struct uvc_conf uvc_conf = {
	.c_data = &uvc_c_data,
	.caps = uvc_caps,
	.formats = uvc_formats,
	...
	.fs_desc = uvc_fs_desc,
	.hs_desc = uvc_hs_desc,
	.str_desc = uvc_str_desc,
	.ctl_desc = uvc_ctl_desc,
	.desc_iad_ifnum = uvc_desc_iad + 2,
	.desc_if_vc_ifnum = uvc_desc_if_vc + 2,
	.desc_if_vc_header_ifnum = uvc_desc_if_vc_header + 12,
	.desc_if_vs_ifnum = uvc_desc_if_vs + 2,
	.desc_if_vs_header_epaddr = uvc_desc_if_vs_header + 6,
	.fs_desc_ep_epaddr = uvc_fs_desc_ep + 2,
	.hs_desc_ep_epaddr = uvc_hs_desc_ep + 2,
};
