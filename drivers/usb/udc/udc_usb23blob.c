...
void usb23_manager_write(const struct device *dev, struct usb23_ep_data *ep_data, uint32_t reg,
			 uint32_t val)
{
	LOG_DBG("%s: ep=%02x usb23_manager=0x%08lx offset=0x%08x", __func__, ep_data->addr,
		ep_data->manager_base, reg);
			   ...
			   struct net_buf *buf, uint32_t ctrl)
{
	...
	LOG_DBG("PUSH %d buf=%p data=%p size=%d", ep_data->head, buf, buf->data, buf->size);
...
static struct net_buf *usb23_pop_trb(const struct device *dev, struct usb23_ep_data *ep_data)
{
	...
	do {
		...
		LOG_DBG("POP %d ep=0x%02x buf=%p", ep_data->addr, ep_data->tail, buf);
...
static void usb23_depcmd_ep_config(const struct device *dev, struct usb23_ep_data *ep_data)
{
	...
	LOG_INF("Configuring endpoint 0x%02x with wMaxPacketSize=%d", ep_data->addr, ep_cfg->mps);
...
{
	LOG_DBG("DepXferConfig: ep=0x%02x", ep_data->addr);
...
{
	LOG_DBG("DepGetState: ep=0x%02x", ep_data->addr);
...
{
	LOG_WRN("DepSetStall: ep=0x%02x", ep_data->addr);
...
{
	LOG_INF("DepClearStall ep=0x%02x", ep_data->addr);
...
static void usb23_depcmd_start_xfer(const struct device *dev, struct usb23_ep_data *ep_data)
{
	...
	LOG_DBG("DepStartXfer done ep=0x%02x xferrscidx=0x%x", ep_data->addr, ep_data->xferrscidx);
}
...
static void usb23_depcmd_update_xfer(const struct device *dev, struct usb23_ep_data *ep_data)
{
	...
	LOG_DBG("DepUpdateXfer done ep=0x%02x addr=0x%08x data=0x%08x", ep_data->addr,
				  ...
				  uint32_t flags)
{
	...
	LOG_DBG("DepEndXfer done ep=0x%02x", ep_data->addr);
...
static void usb23_depcmd_start_config(const struct device *dev, struct usb23_ep_data *ep_data)
{
	...
	LOG_DBG("DepStartConfig done ep=0x%02x", ep_data->addr);
}
...
static void usb23_trb_ctrl_setup_out(const struct device *dev)
{
	...
	LOG_DBG("TRB_CONTROL_SETUP ep=0x%02x size=%d", USB_CONTROL_EP_OUT, buf->size);
				 ...
				 struct net_buf *buf)
{
	...
	LOG_DBG("TRB_BULK_NATIVE ep=0x%02x buf=%p data=%p size=%d len=%d", ep_data->addr, buf,
...
static void usb23_process_queue(const struct device *dev, struct usb23_ep_data *ep_data)
{
	...
	do {
		buf = udc_buf_peek(dev, ep_data->addr);
		if (buf == NULL) {
			break;
		}
		...
		(void)udc_buf_get(dev, ep_data->addr);
	} while (ret == 0);
}
...
static void usb23_on_usb_reset(const struct device *dev)
{
	...
	for (uint8_t addr = 1; addr < config->num_bidir_eps; addr++) {
...
/* OUT */
static void usb23_on_ctrl_write_setup(const struct device *dev, struct usb23_ep_data *ep_data,
				      struct net_buf *buf)
{
	LOG_DBG("CTRL_WRITE_SETUP (out) buf=%p", buf);
	usb23_trb_ctrl_data_out(dev);
}

/* OUT */
static void usb23_on_ctrl_write_data(const struct device *dev, struct usb23_ep_data *ep_data,
				     struct net_buf *buf)
{
	int ret;

	LOG_DBG("CTRL_WRITE_DATA (out) buf=%p", buf);
...
/* IN */
static void usb23_on_ctrl_write_status(const struct device *dev, struct usb23_ep_data *ep_data,
				       struct net_buf *buf)
{
	int ret;

	LOG_DBG("CTRL_WRITE_STATUS (in) buf=%p", buf);
...
/* OUT */
static void usb23_on_ctrl_read_setup(const struct device *dev, struct usb23_ep_data *ep_data,
				     struct net_buf *buf)
{
	int ret;

	LOG_DBG("CTRL_READ_SETUP (out) buf=%p", buf);
	ret = udc_ctrl_submit_s_in_status(dev);
	__ASSERT_NO_MSG(ret == 0);
}

/* IN */
static void usb23_on_ctrl_read_data(const struct device *dev, struct usb23_ep_data *ep_data,
				    struct net_buf *buf)
{
	LOG_DBG("CTRL_READ_DATA (in) buf=%p", buf);
	usb23_trb_ctrl_status_3_out(dev);
	udc_ctrl_update_stage(dev, buf);
}

/* OUT */
static void usb23_on_ctrl_read_status(const struct device *dev, struct usb23_ep_data *ep_data,
				      struct net_buf *buf)
{
	int ret;

	LOG_DBG("CTRL_READ_STATUS (out) buf=%p", buf);
	ret = udc_ctrl_submit_status(dev, buf);
	__ASSERT_NO_MSG(ret == 0);
	udc_ctrl_update_stage(dev, buf);
}
...
/* OUT */
static void usb23_on_ctrl_nodata_setup(const struct device *dev, struct usb23_ep_data *ep_data,
				       struct net_buf *buf)
{
	int ret;

	LOG_DBG("CTRL_NODATA_SETUP (out) buf=%p", buf);
	ret = udc_ctrl_submit_s_status(dev);
	__ASSERT_NO_MSG(ret == 0);
}

/* IN */
static void usb23_on_ctrl_nodata_status(const struct device *dev, struct usb23_ep_data *ep_data,
					struct net_buf *buf)
{
	int ret;

	LOG_DBG("CTRL_NODATA_STATUS (in) buf=%p", buf);
	ret = udc_ctrl_submit_status(dev, buf);
	__ASSERT_NO_MSG(ret == 0);
	udc_ctrl_update_stage(dev, buf);
}

static void usb23_on_ctrl_setup(const struct device *dev, struct usb23_ep_data *ep_data,
				struct net_buf *buf)
{
	struct usb23_data *priv = udc_get_private(dev);
	...
	if (udc_ctrl_stage_is_data_out(dev)) {
		usb23_on_ctrl_write_setup(dev, ep_data, buf);
	} else if (udc_ctrl_stage_is_data_in(dev)) {
		usb23_on_ctrl_read_setup(dev, ep_data, buf);
	} else if (udc_ctrl_stage_is_no_data(dev)) {
		usb23_on_ctrl_nodata_setup(dev, ep_data, buf);
	} else {
		LOG_ERR("unknown setup stage");
	}
}

static void usb23_on_ctrl_data(const struct device *dev, struct usb23_ep_data *ep_data,
			       struct net_buf *buf)
{
	if (udc_ctrl_stage_is_data_out(dev)) {
		__ASSERT_NO_MSG(ep_data->addr == USB_CONTROL_EP_OUT);
		usb23_on_ctrl_write_data(dev, ep_data, buf);
	} else if (udc_ctrl_stage_is_data_in(dev)) {
		__ASSERT_NO_MSG(ep_data->addr == USB_CONTROL_EP_IN);
		usb23_on_ctrl_read_data(dev, ep_data, buf);
		net_buf_unref(buf);
	} else {
...
static void usb23_on_ctrl_status(const struct device *dev, struct usb23_ep_data *ep_data,
				 struct net_buf *buf)
{
	if (udc_ctrl_stage_is_status_in(dev)) {
		usb23_on_ctrl_write_status(dev, ep_data, buf);
	} else if (udc_ctrl_stage_is_status_out(dev)) {
		usb23_on_ctrl_read_status(dev, ep_data, buf);
	} else if (udc_ctrl_stage_is_no_data(dev)) {
		usb23_on_ctrl_nodata_status(dev, ep_data, buf);
	} else {
 ...
 */
static void usb23_on_xfer_not_ready(const struct device *dev, struct usb23_ep_data *ep_data,
				    uint32_t status)
{
	switch (status & USB23_DEPEVT_STATUS_B3_MASK) {
	case USB23_DEPEVT_STATUS_B3_CONTROL_SETUP:
		LOG_EVENT(USB23_DEPEVT_TYPE_XFERNOTREADY_CONTROL_SETUP);
		break;
	case USB23_DEPEVT_STATUS_B3_CONTROL_DATA:
		LOG_EVENT(USB23_DEPEVT_TYPE_XFERNOTREADY_CONTROL_DATA);
		break;
	case USB23_DEPEVT_STATUS_B3_CONTROL_STATUS:
		LOG_EVENT(USB23_DEPEVT_TYPE_XFERNOTREADY_CONTROL_STATUS);
		break;
	}
	return;
}

static void usb23_on_xfer_done_norm(const struct device *dev, struct usb23_ep_data *ep_data)
{
	...
	LOG_DBG("ep=0x%02x buf=%p", ep_data->addr, buf);
	__ASSERT_NO_MSG(buf != NULL);
...
static void usb23_on_xfer_done_ctrl(const struct device *dev, struct usb23_ep_data *ep_data)
{
	...
	if (ep_data->addr == USB_CONTROL_EP_OUT) {
	...
	case USB23_TRB_CTRL_TRBCTL_CONTROL_SETUP:
		usb23_on_ctrl_setup(dev, ep_data, buf);
		break;
	case USB23_TRB_CTRL_TRBCTL_CONTROL_DATA:
		usb23_on_ctrl_data(dev, ep_data, buf);
		break;
	case USB23_TRB_CTRL_TRBCTL_CONTROL_STATUS_2:
		usb23_on_ctrl_status(dev, ep_data, buf);
		break;
...
{
	struct usb23_ep_data *ep_data = usb23_get_ep_data(dev, usb23_get_addr(depevt.ep_num));
	...
	case USB23_DEPEVT_TYPE_XFERNOTREADY:
		usb23_on_xfer_not_ready(dev, ep_data, depevt.status);
		break;
...
void usb23_on_event(const struct device *dev)
{
	...
	for (struct usb23_ep_data **ep_data = config->manager_list; *ep_data != NULL; ep_data++) {
		...
		reg = usb23_manager_read(dev, *ep_data, USB23_MANAGER_CONTROLSTATUS);
		reg &= USB23_MANAGER_CONTROLSTATUS_FSMSTATE_MASK;
		if (reg == USB23_MANAGER_CONTROLSTATUS_FSMSTATE_IDLE) {
...
int usb23_api_ep_set_halt(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	...
	LOG_WRN("Setting stall state on endpoint ep=0x%02x", ep_data->addr);
...
int usb23_api_ep_clear_halt(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	...
	LOG_DBG("Clearing stall state on endpoint ep=0x%02x", ep_data->addr);
	__ASSERT_NO_MSG(ep_data->addr != USB_CONTROL_EP_OUT);
	__ASSERT_NO_MSG(ep_data->addr != USB_CONTROL_EP_IN);
...
void usb23_dump_registers(const struct device *dev, const struct shell *sh)
{
	for (int i = 0; i < ARRAY_SIZE(usb23_regs); i++) {
		...
		if (data != ureg->last) {
			shell_print(sh, "reg 0x%08x == 0x%08x  (was 0x%08x)  // %s", ureg->addr,
...
void usb23_dump_link_state(const struct device *dev, const struct shell *sh)
{
	...
	switch (reg & USB23_DSTS_CONNECTSPD_MASK) {
	case USB23_DSTS_CONNECTSPD_HS:
		shell_print(sh, "USB23_DSTS_CONNECTSPD_HS");
		goto usb2;
	case USB23_DSTS_CONNECTSPD_FS:
		shell_print(sh, "USB23_DSTS_CONNECTSPD_FS");
		goto usb2;
	usb2:
		switch (reg & USB23_DSTS_USBLNKST_MASK) {
		case USB23_DSTS_USBLNKST_USB2_ON_STATE:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB2_ON_STATE");
			break;
		case USB23_DSTS_USBLNKST_USB2_SLEEP_STATE:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB2_SLEEP_STATE");
			break;
		case USB23_DSTS_USBLNKST_USB2_SUSPEND_STATE:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB2_SUSPEND_STATE");
			break;
		case USB23_DSTS_USBLNKST_USB2_DISCONNECTED:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB2_DISCONNECTED");
			break;
		case USB23_DSTS_USBLNKST_USB2_EARLY_SUSPEND:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB2_EARLY_SUSPEND");
			break;
		case USB23_DSTS_USBLNKST_USB2_RESET:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB2_RESET");
			break;
		case USB23_DSTS_USBLNKST_USB2_RESUME:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB2_RESUME");
			break;
		}
		break;
	case USB23_DSTS_CONNECTSPD_SS:
		shell_print(sh, "USB23_DSTS_CONNECTSPD_SS");

		switch (reg & USB23_DSTS_USBLNKST_MASK) {
		case USB23_DSTS_USBLNKST_USB3_U0:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_U0");
			break;
		case USB23_DSTS_USBLNKST_USB3_U1:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_U1");
			break;
		case USB23_DSTS_USBLNKST_USB3_U2:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_U2");
			break;
		case USB23_DSTS_USBLNKST_USB3_U3:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_U3");
			break;
		case USB23_DSTS_USBLNKST_USB3_SS_DIS:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_SS_DIS");
			break;
		case USB23_DSTS_USBLNKST_USB3_RX_DET:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_RX_DET");
			break;
		case USB23_DSTS_USBLNKST_USB3_SS_INACT:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_SS_INACT");
			break;
		case USB23_DSTS_USBLNKST_USB3_POLL:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_POLL");
			break;
		case USB23_DSTS_USBLNKST_USB3_RECOV:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_RECOV");
			break;
		case USB23_DSTS_USBLNKST_USB3_HRESET:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_HRESET");
			break;
		case USB23_DSTS_USBLNKST_USB3_CMPLY:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_CMPLY");
			break;
		case USB23_DSTS_USBLNKST_USB3_LPBK:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_LPBK");
			break;
		case USB23_DSTS_USBLNKST_USB3_RESET_RESUME:
			shell_print(sh, "USB23_DSTS_USBLNKST_USB3_RESET_RESUME");
			break;
		}
		break;
	default:
...
void usb23_dump_events(const struct device *dev, const struct shell *sh)
{
	...
	for (int i = 0; i < CONFIG_USB23_EVT_NUM; i++) {
		...
		shell_print(sh, "evt 0x%02x: 0x%08x%s", i, evt.raw, s);
	}
}
		     ...
		     const struct shell *sh)
{
	...
	for (int i = 0; i < ep_data->num_of_trbs; i++) {
		...
		shell_print(
			sh,
			"%02d ep=0x%02x addr=0x%08x%08x ctl=%ld sts=%ld hwo=%d lst=%d chn=%d"
			" csp=%d isp=%d ioc=%d spr=%d pcm1=%ld sof=%ld bufsiz=%ld%s%s%s%s",
...
#define DUMP(reg)                                                                                  \
	shell_print(sh, "%-30s [0x%04x] 0x%08x", #reg, reg, usb23_manager_read(dev, ep_data, reg))
	...
	case USB23_MANAGER_CONTROLSTATUS_FSMSTATE_IDLE:
		shell_print(sh, "USB23_MANAGER_CONTROLSTATUS_FSMSTATE_IDLE");
		break;
	case USB23_MANAGER_CONTROLSTATUS_FSMSTATE_WAIT:
		shell_print(sh, "USB23_MANAGER_CONTROLSTATUS_FSMSTATE_WAIT");
		break;
	case USB23_MANAGER_CONTROLSTATUS_FSMSTATE_FIFOCHECK:
		shell_print(sh, "USB23_MANAGER_CONTROLSTATUS_FSMSTATE_FIFOCHECK");
		break;
	case USB23_MANAGER_CONTROLSTATUS_FSMSTATE_RTRBCTRL:
		shell_print(sh, "USB23_MANAGER_CONTROLSTATUS_FSMSTATE_RTRBCTRL");
		break;
	case USB23_MANAGER_CONTROLSTATUS_FSMSTATE_WTRBCTRL:
		shell_print(sh, "USB23_MANAGER_CONTROLSTATUS_FSMSTATE_WTRBCTRL");
		break;
	case USB23_MANAGER_CONTROLSTATUS_FSMSTATE_RTRBSIZE:
		shell_print(sh, "USB23_MANAGER_CONTROLSTATUS_FSMSTATE_RTRBSIZE");
		break;
	case USB23_MANAGER_CONTROLSTATUS_FSMSTATE_WTRBSIZE:
		shell_print(sh, "USB23_MANAGER_CONTROLSTATUS_FSMSTATE_WTRBSIZE");
		break;
	case USB23_MANAGER_CONTROLSTATUS_FSMSTATE_RINGDOORBELL:
		shell_print(sh, "USB23_MANAGER_CONTROLSTATUS_FSMSTATE_RINGDOORBELL");
		break;
	case USB23_MANAGER_CONTROLSTATUS_FSMSTATE_FIFOEN:
		shell_print(sh, "USB23_MANAGER_CONTROLSTATUS_FSMSTATE_FIFOEN");
		break;
	case USB23_MANAGER_CONTROLSTATUS_FSMSTATE_DONE:
		shell_print(sh, "USB23_MANAGER_CONTROLSTATUS_FSMSTATE_DONE");
		break;
	}

	shell_print(sh, "USB23_MANAGER_CONTROLSTATUS_TRBID=%ld",
...
#undef R
	...
	for (size_t i = 0; i < sizeof(fifo) / sizeof(*fifo); i++) {
		...
		shell_print(sh, "%-15s [%d] = %d", fifo[i].name, fifo[i].num, reg);
	}
}
...
static int cmd_usb23_thres(const struct shell *sh, size_t argc, char **argv)
{
	usbm_fifothres = strtol(argv[1], NULL, 10);
	return 0;
}
