...
static void video_shell_print_buffer(const struct shell *sh, struct video_buffer *vbuf,
				     struct video_format *fmt, uint32_t num_frames,
				     uint32_t frmrate_fps, uint32_t frmival_msec)
{
	uint32_t line_offset = vbuf->line_offset;

	shell_print(sh, "%s %u at %u ms, %s %3lu/%lu, %s %u-%u/%u, %s %u-%u/%u, %s %u FPS %u ms",
		    "Buffer", i, vbuf->timestamp,
		    "Frame", num_frames, line_offset * fmt.pitch,
		    "Lines", line_offset * fmt.pitch + vbuf->bytesused, size, line_offset,
		    "Bytes", line_offset + vbuf->bytesused / fmt.pitch, fmt.height,
...
static int cmd_video_capture(const struct shell *sh, size_t argc, char **argv)
{
	...
	for (unsigned long i = 0; i < num_frames;) {
		...
		video_shell_print_buffer(sh, vbuf, fmt, num_frames, frmrate_fps, frmival_msec);
