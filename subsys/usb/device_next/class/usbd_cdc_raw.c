...
struct cdc_raw_data {
	...
	bool line_state;
	/* Interface string so Windows can bind the composite function */
	struct usbd_desc_node *if_str_desc;
};

uint8_t usbd_cdc_raw_get_bulk_in(const struct device *dev)
{
...
int cdc_raw_write(const struct device *dev, struct net_buf *buf, bool zlp)
{
	...
	memset(bi, 0, sizeof(struct udc_buf_info));
	bi->ep = usbd_cdc_raw_get_bulk_in(dev);
...
static int usbd_cdc_raw_init(struct usbd_class_data *const c_data)
{
	...
	/*
	 * priv-tvai-usb 2902189 / 23f990a: unlabeled CDC-RAW made Windows
	 * fail the whole composite, so usbvideo.sys never bound UVC.
	 * Use usbd_str_desc_get_idx (ACM path); the upstream patch's
	 * ->str.idx / `== true` did not assign the index.
	 */
	if (data->if_str_desc != NULL && desc->if0.iInterface == 0) {
		if (usbd_add_descriptor(uds_ctx, data->if_str_desc)) {
			LOG_WRN("Failed to add CDC-RAW interface string descriptor");
		} else {
			const uint8_t idx = usbd_str_desc_get_idx(data->if_str_desc);
				...
				struct net_buf *buf, int err)
{
	...
	if (bi->ep == usbd_cdc_raw_get_bulk_in(dev) && data->write_callback) {
...
#define USBD_CDC_RAW_DT_DEVICE_DEFINE(n)					\
	...
	CDC_ACM_DEFINE_DESCRIPTOR(n);						\
										\
	USBD_DESC_STRING_DEFINE(cdc_raw_if_str_desc_##n,			\
				"CDC_RAW",					\
	...
	static struct cdc_raw_data data_##n = {					\
		...
		.if_str_desc = &cdc_raw_if_str_desc_##n,			\
	};									\
										\
