...
#define GC2145_REG8(addr)  ((uint32_t)(addr) | VIDEO_REG_ADDR8_DATA8)
#define GC2145_REG16(addr) ((uint32_t)(addr) | VIDEO_REG_ADDR8_DATA16_BE)

/* System registers, no pagination */

/* Paginated registers, Page 0 */

#define GC2145_P0_AMODE1		GC2145_REG8(0x17)

#define GC2145_REG_AMODE1		GC2145_REG8(0x17)
#define GC2145_AMODE1_UPDOWN		BIT(1)
#define GC2145_AMODE1_MIRROR		BIT(0)

#define GC2145_REG_OUTPUT_FMT		GC2145_REG8(0x84)
#define GC2145_OUTPUT_FMT_MASK		0x1F
#define GC2145_OUTPUT_FMT_RGB565	0x06
#define GC2145_OUTPUT_FMT_YCBYCR	0x02

#define GC2145_REG_SYNC_MODE		0x86 /* REG8 */
#define GC2145_SYNC_MODE_DEF		0x23
#define GC2145_SYNC_MODE_COL_SWITCH	0x10
#define GC2145_SYNC_MODE_ROW_SWITCH	0x20

#define GC2145_REG_RESET		GC2145_REG8(0xFE)
#define GC2145_RESET_SW			0x80

#define GC2145_REG_PAD_IO		GC2145_REG8(0xF2)
#define GC2145_PAD_IO_ALL_OUT		0x0F
#define GC2145_PAD_IO_ALL_IN		0x00

#define GC2145_REG_CHIP_ID		GC2145_REG16(0xF0)
#define GC2145_SET_P0_REGS		0x00

#define GC2145_REG_CROP_ENABLE		GC2145_REG8(0x90)
#define GC2145_CROP_SET_ENABLE		0x01

#define GC2145_REG_BLANK_WINDOW_BASE	0x09 /* base address */

#define GC2145_REG_WINDOW_BASE		0x91 /* base address */
...
#define GC2145_REG_ROW_START(addr)	GC2145_REG16((addr) + 0)
#define GC2145_REG_COL_START(addr)	GC2145_REG16((addr) + 2)
#define GC2145_REG_WIN_HEIGHT(addr)	GC2145_REG16((addr) + 4)
#define GC2145_REG_WIN_WIDTH(addr)	GC2145_REG16((addr) + 6)
...
static int gc2145_soft_reset(const struct device *dev)
{
	...
	ret = video_write_cci_reg(&cfg->i2c, GC2145_REG_RESET, GC2145_RESET_SW);
...
static int gc2145_set_ctrl_vflip(const struct device *dev, bool enable)
{
	...
	return video_modify_cci_reg(&cfg->i2c, GC2145_REG_AMODE1, GC2145_AMODE1_UPDOWN,
				   enable ? 0xff : 0x00);
}
...
static int gc2145_set_ctrl_hmirror(const struct device *dev, bool enable)
{
	...
	return video_modify_cci_reg(&cfg->i2c, GC2145_REG_AMODE1, GC2145_AMODE1_MIRROR,
				    enable ? 0xff : 0x00);
}
			     ...
			     uint32_t y_offset, uint32_t window_width, uint32_t window_height)
{
	...
	ret = video_write_cci_reg(&cfg->i2c, GC2145_REG_ROW_START(reg), y_offset);
	if (ret < 0) {
		return ret;
	}

	ret = video_write_cci_reg(&cfg->i2c, GC2145_REG_COL_START(reg), x_offset);
	if (ret < 0) {
		return ret;
	}

	ret = video_write_cci_reg(&cfg->i2c, GC2145_REG_WIN_HEIGHT(reg), window_height);
	if (ret < 0) {
		return ret;
	}

	ret = video_write_cci_reg(&cfg->i2c, GC2145_REG_WIN_WIDTH(reg), window_width);
	if (ret < 0) {
		return ret;
	}
...
static int gc2145_set_output_format(const struct device *dev, int output_format)
{
	...
	if (output_format == VIDEO_PIX_FMT_RGB565) {
		output_format = GC2145_OUTPUT_FMT_RGB565;
	} else if (output_format == VIDEO_PIX_FMT_YUYV) {
		output_format = GC2145_OUTPUT_FMT_YCBYCR;
	} else {
		LOG_ERR("Image format not supported");
		return -ENOTSUP;
	}

	ret = video_modify_cci_reg(&cfg->i2c, GC2145_REG_OUTPUT_FMT, GC2145_OUTPUT_FMT_MASK,
				   output_format);
	if (ret < 0) {
		return ret;
	}
...
static int gc2145_set_resolution(const struct device *dev, uint32_t w, uint32_t h)
{
	...
	ret = gc2145_set_window(dev, GC2145_REG_BLANK_WINDOW_BASE, win_x, win_y,
				win_w + 16, win_h + 8);
	if (ret < 0) {
		return ret;
	}
...
static int gc2145_set_stream(const struct device *dev, bool enable)
{
	...
	return enable ? video_write_cci_reg(&cfg->i2c, GC2145_REG_PAD_IO, GC2145_PAD_IO_ALL_OUT)
		      : video_write_cci_reg(&cfg->i2c, GC2145_REG_PAD_IO, GC2145_PAD_IO_ALL_IN);
}
