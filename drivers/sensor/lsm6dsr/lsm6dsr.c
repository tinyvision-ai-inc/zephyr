...
static const uint16_t lsm6dsr_odr_map[] = {0, 12, 26, 52, 104, 208, 417, 833,
					1667, 3333, 6667};
...
static int lsm6dsr_odr_to_freq_val(uint16_t odr)
{
	...
	return lsm6dsr_odr_map[ARRAY_SIZE(lsm6dsr_odr_map) - 1];
}

static const uint16_t lsm6dsr_accel_fs_map[] = {2, 16, 4, 8};

static int lsm6dsr_accel_range_to_fs_val(int32_t range, bool double_range)
{
	size_t i;

	for (i = 0; i < ARRAY_SIZE(lsm6dsr_accel_fs_map); i++) {
		if (range == (lsm6dsr_accel_fs_map[i] << double_range)) {
			return i;
		}
	}
...
static int lsm6dsr_accel_fs_val_to_gain(int fs, bool double_range)
{
	/* Range of ±2G has a LSB of GAIN_UNIT_XL, thus divide by 2 */
	return double_range ?
		lsm6dsr_accel_fs_map[fs] * GAIN_UNIT_XL :
		lsm6dsr_accel_fs_map[fs] * GAIN_UNIT_XL / 2;
}

static const uint16_t lsm6dsr_gyro_fs_map[] = {250, 125, 500, 0, 1000, 0, 2000};
static const uint16_t lsm6dsr_gyro_fs_sens[] = {2, 1, 4, 0, 8, 0, 16};
...
static int lsm6dsr_accel_range_set(const struct device *dev, int32_t range)
{
	...
	fs = lsm6dsr_accel_range_to_fs_val(range, range_double);
	if (fs < 0) {
		return fs;
	}
	...
	data->acc_gain = lsm6dsr_accel_fs_val_to_gain(fs, range_double);
	return 0;
}
...
static int lsm6dsr_gyro_range_set(const struct device *dev, int32_t range)
{
	...
	data->gyro_gain = (lsm6dsr_gyro_fs_sens[fs] * GAIN_UNIT_G);
	return 0;
}
...
static int lsm6dsr_attr_set(const struct device *dev,
			    ...
			    const struct sensor_value *val)
{
#if defined(CONFIG_LSM6DSR_SENSORHUB)
	struct lsm6dsr_data *data = dev->data;
#endif /* CONFIG_LSM6DSR_SENSORHUB */
	...
	case SENSOR_CHAN_HUMIDITY:
		...
		return lsm6dsr_shub_config(dev, chan, attr, val);
#endif /* CONFIG_LSM6DSR_SENSORHUB */
	default:
...
#if defined(CONFIG_LSM6DSR_SENSORHUB)
static int lsm6dsr_sample_fetch_shub(const struct device *dev)
{
	if (lsm6dsr_shub_fetch_external_devs(dev) < 0) {
...
#if defined(CONFIG_LSM6DSR_SENSORHUB)
static inline void lsm6dsr_magn_convert(struct sensor_value *val, int raw_val,
...
static inline int lsm6dsr_magn_get_channel(enum sensor_channel chan,
					   struct sensor_value *val,
					   struct lsm6dsr_data *data)
{
	int16_t sample[3];
	...
	idx = lsm6dsr_shub_get_idx(data->dev, SENSOR_CHAN_MAGN_XYZ);
	if (idx < 0) {
	...
	case SENSOR_CHAN_MAGN_X:
		lsm6dsr_magn_convert(val, sample[0], data->magn_gain);
		break;
	case SENSOR_CHAN_MAGN_Y:
		lsm6dsr_magn_convert(val, sample[1], data->magn_gain);
		break;
	case SENSOR_CHAN_MAGN_Z:
		lsm6dsr_magn_convert(val, sample[2], data->magn_gain);
		break;
	case SENSOR_CHAN_MAGN_XYZ:
		lsm6dsr_magn_convert(val, sample[0], data->magn_gain);
		lsm6dsr_magn_convert(val + 1, sample[1], data->magn_gain);
		lsm6dsr_magn_convert(val + 2, sample[2], data->magn_gain);
		break;
	default:
		return -ENOTSUP;
	}
...
static inline void lsm6dsr_hum_convert(struct sensor_value *val,
				       struct lsm6dsr_data *data)
{
	float rh;
	int16_t raw_val;
	...
	idx = lsm6dsr_shub_get_idx(data->dev, SENSOR_CHAN_HUMIDITY);
	if (idx < 0) {
...
static inline void lsm6dsr_press_convert(struct sensor_value *val,
					 struct lsm6dsr_data *data)
{
	int32_t raw_val;
	int idx;

	idx = lsm6dsr_shub_get_idx(data->dev, SENSOR_CHAN_PRESS);
	if (idx < 0) {
		LOG_DBG("external press/temp not supported");
		return;
	}
...
static inline void lsm6dsr_temp_convert(struct sensor_value *val,
					struct lsm6dsr_data *data)
{
	int16_t raw_val;
	int idx;
			       ...
			       struct sensor_value *val)
{
	...
	case SENSOR_CHAN_MAGN_XYZ:
		...
		lsm6dsr_magn_get_channel(chan, val, data);
		break;
	...
	case SENSOR_CHAN_PRESS:
		...
		lsm6dsr_press_convert(val, data);
		break;
...
static int lsm6dsr_init_chip(const struct device *dev)
{
	 ...
	 * register and the register used for a SW reset.  If the lsm6dsr wasn't on the user
	 * bank when it reset, then both the chip id check and the sw reset will fail unless we
	 * set the bank now.
	 */
	if (lsm6dsr_mem_bank_set(ctx, LSM6DSR_USER_BANK) < 0) {
	 ...
	 */
	if (lsm6dsr_sh_master_get(ctx, &master_on) < 0) {
		LOG_DBG("Failed to get I2C_MASTER status");
		return -EIO;
	}
	if (master_on) {
		LOG_DBG("Disable shub before reset");
		lsm6dsr_sh_master_set(ctx, 0);
		k_busy_wait(300);
	}

	/* reset device */
	if (lsm6dsr_reset_set(ctx, 1) < 0) {
		return -EIO;
	}
	...
	default:
	case 0:
		lsm6dsr_xl_power_mode_set(ctx, LSM6DSR_HIGH_PERFORMANCE_MD);
		break;
	case 1:
		lsm6dsr_xl_power_mode_set(ctx, LSM6DSR_LOW_NORMAL_POWER_MD);
		break;
	}
	...
	lsm6dsr->acc_gain = lsm6dsr_accel_fs_val_to_gain(fs, cfg->accel_range & ACCEL_RANGE_DOUBLE);

	odr = cfg->accel_odr;
	LOG_DBG("accel odr is %d", odr);
	lsm6dsr->accel_freq = lsm6dsr_odr_to_freq_val(odr);
	if (lsm6dsr_accel_set_odr_raw(dev, odr) < 0) {
	...
	default:
	case 0:
		lsm6dsr_gy_power_mode_set(ctx, LSM6DSR_GY_HIGH_PERFORMANCE);
		break;
	case 1:
		lsm6dsr_gy_power_mode_set(ctx, LSM6DSR_GY_NORMAL);
		break;
	}
	...
	lsm6dsr->gyro_gain = (lsm6dsr_gyro_fs_sens[fs] * GAIN_UNIT_G);

	odr = cfg->gyro_odr;
	LOG_DBG("gyro odr is %d", odr);
	lsm6dsr->gyro_freq = lsm6dsr_odr_to_freq_val(odr);
	if (lsm6dsr_gyro_set_odr_raw(dev, odr) < 0) {
...
#ifdef CONFIG_LSM6DSR_SENSORHUB
	data->shub_inited = true;
	if (lsm6dsr_shub_init(dev) < 0) {
...
#define LSM6DSR_CONFIG_COMMON(inst)					\
	...
	.accel_range = DT_INST_PROP(inst, accel_range) |		\
		(DT_NODE_HAS_COMPAT(DT_DRV_INST(inst), st_lsm6dsr32) ?	\
