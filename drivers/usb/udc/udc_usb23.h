...
struct usb23_config {
	...
	struct usb23_ep_data **manager_list;
...
struct usb23_ep_data {
	...
	/* Endpoint number (physical address) and address (logical address) */
	uint8_t epn;
	uint8_t addr;
};
 ...
 * To:   0,    1,    2,    3,    4,    5,    6,    7,    ...
 */
static inline uint8_t usb23_get_epn(uint8_t addr)
{
	return ((addr & 0x7f) << 1) | ((addr & 0x80) >> 7);
}

static inline uint8_t usb23_get_addr(uint8_t epn)
{
	return (((epn & 0xfe) >> 1) | (epn & 0x01) << 7);
}

static inline struct usb23_ep_data *usb23_get_ep_data(const struct device *dev, uint8_t addr)
{
	const struct usb23_config *config = dev->config;

	return &config->ep_data[usb23_get_epn(addr)];
}
