...
#define VIDEO_STATS_CHANNELS_Y BIT(2)

/** Bitmap to ask for any statistics filling a @struct video_stats_channels */
#define VIDEO_STATS_CHANNELS (VIDEO_STATS_CHANNELS_RGB | VIDEO_STATS_CHANNELS_Y)

/** Statistics in the form of an histogram, with R, G, B channels filled. */
#define VIDEO_STATS_HISTOGRAM_RGB BIT(3)

/** Statistics in the form of an histogram. Y (lum) channel filled. */
#define VIDEO_STATS_HISTOGRAM_Y BIT(4)
...
struct video_stats_histogram {
	/** Base structure with fields common to all types of statistics. */
	struct video_stats base;
	/** Storage for the histogram buckets for R, G, B channels. */
	uint16_t *buckets_rgb[3];
	/** Storage for the histogram buckets for y channel. */
	uint16_t *buckets_y;
	/** Number of buckets effectively used. */
	size_t num_buckets;
	/** Total number of values added to the buckets. */
	uint32_t num_values;
};
 ...
 * the matching size fields to the size effectively used.
 *
